...
const livroRepository = {
    ...
    // Usado pelo módulo de Empréstimos (Membro 3) para controlar a disponibilidade do livro
    diminuirEstoque: async (id) => {
        const query = 'UPDATE livros SET quantidade_estoque = quantidade_estoque - 1 WHERE id = $1 RETURNING *';
        const res = await pool.query(query, [id]);
        return res.rows[0];
    },
    aumentarEstoque: async (id) => {
        const query = 'UPDATE livros SET quantidade_estoque = quantidade_estoque + 1 WHERE id = $1 RETURNING *';
        const res = await pool.query(query, [id]);
        return res.rows[0];
    }
